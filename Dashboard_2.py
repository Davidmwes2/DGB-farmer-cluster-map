...
# --- CLUSTERING LOGIC (District Bound Only) ---
def assign_stable_clusters(data, target_size=40, min_size=10):
    processed_list = []
    # Cluster strictly within District only
    for dist, dist_data in data.groupby('District'):
        ...
        # Determine K to satisfy Min 10 / Max 40
        if total <= min_size:
            n_clusters = 1
        else:
        ...
        # ID Format: UG-DIST-001 (Subcounty removed from ID as it's no longer a boundary)
...
def create_pdf(dataframe, lead_df):
    ...
    for cluster in sorted(dataframe['Cluster_Label'].unique()):
        ...
        pdf.cell(190, 10, f" LEAD FARMER: {lead['Lead_Farmer']} | COUNT: {lead['Farmer_Count']}", ln=True, fill=True)
        pdf.set_text_color(0, 0, 0); pdf.ln(2)
        members = dataframe[dataframe['Cluster_Label'] == cluster]
        for _, row in members.iterrows():
            pdf.cell(90, 8, str(row['Name'])[:40], border=1); pdf.cell(60, 8, str(row['Subcounty']), border=1); pdf.cell(40, 8, f"{row['Latitude']:.4f}", border=1); pdf.ln()
    return pdf.output(dest='S').encode('latin-1')

if st.sidebar.button("🛠️ Prepare PDF Handbook"):
    pdf_bytes = create_pdf(display_df, lead_farmer_df)
    st.sidebar.download_button("📥 Download PDF List", pdf_bytes, f"Farmer_Handbook.pdf", "application/pdf")

csv_data = lead_farmer_df.to_csv(index=False).encode('utf-8')
st.sidebar.download_button("📍 Download Cluster CSV", csv_data, "Cluster_Coordinates.csv", "text/csv")

# --- MAP ---
st.title("🚜 Farmer Geographic Intelligence Hub")
...
for _, row in lead_farmer_df.iterrows():
    ...
    # This creates the "Banner" effect on hover
    banner_html = f"""
    <div style="font-size: 12pt; width: 150px;">
        <b>Cluster:</b> {row['Cluster_ID']}<br>
        <b>Farmers:</b> {row['Farmer_Count']}<br>
        <b>Lead:</b> {row['Lead_Farmer']}
    </div>
    """
    folium.Marker(
        location=[row['Latitude'], row['Longitude']],
        icon=folium.DivIcon(html=f'<div style="font-family: sans-serif; color: white; background: #000; border-radius: 50%; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; font-size: 9pt; font-weight: bold; border: 2px solid white;">{int(label_num)}</div>'),
